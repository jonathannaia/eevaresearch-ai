"""SQLite-backed SignalRepository — mirrors
src/data_access/live/radar_signal_repository.py exactly in shape:
Signals are never stored, only derived, on every call, from whichever
candidates currently have status=PUBLISHED. No signals table exists in
this schema (see schema.py) and none should ever be added — reusing
src/logic/signal_promotion.py's existing, unmodified
is_eligible_for_signal()/candidate_to_signal() is what guarantees this
backend can never produce a Signal the JSON-backed RadarSignalRepository
wouldn't also produce from the same underlying candidate data."""
