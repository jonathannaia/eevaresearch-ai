...
def test_force_refresh_true_bypasses_a_cached_parse_failure(tmp_path):
    # document_service's own mechanism stays status-agnostic — the
    # narrowing to RETRIEVAL_FAILED-only lives in the caller
    # (radar_pipeline.process_candidate), not here.
    client = MagicMock()
