...
def test_coverage_page_summary_metrics_match_registry_counts():
    ...
    # 62). DISCOVERY_STUBS itself is untouched by that batch (still 25).
    at = _run()
    metrics = {m.label: m.value for m in at.metric}
    assert metrics["Active seed issuers"] == "62"
    assert metrics["Discovery proposals"] == "25"
    assert metrics["Scan-eligible"] == "62"
