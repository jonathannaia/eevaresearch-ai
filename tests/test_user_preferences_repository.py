...
def test_sqlite_v21_is_registered_immediately_after_v20_and_is_current():
    assert schema.CURRENT_SCHEMA_VERSION == 21
    versions = [v for v, _ in schema._MIGRATIONS]
    assert versions == sorted(versions) and len(set(versions)) == len(versions)
    assert versions[-2:] == [20, 21]
...
def test_sqlite_v20_database_upgrades_to_v21_without_touching_user_accounts():
    ...
    assert schema.migrate(conn) == 21  # idempotent
...
def test_postgres_v23_is_registered_immediately_after_v22_and_is_current():
    assert postgres_schema.CURRENT_SCHEMA_VERSION == 23
    versions = [v for v, _ in postgres_schema._MIGRATIONS]
    assert versions == sorted(versions) and len(set(versions)) == len(versions)
    assert versions[-2:] == [22, 23]
...
def test_postgres_v22_database_upgrades_to_v23_without_touching_existing_tables(pg_isolated_connection):
    ...
    assert postgres_schema.migrate(conn) == 23
    ...
    assert postgres_schema.migrate(conn) == 23  # idempotent
